...
class EnhanceNet(nn.Module):
    ...
    def forward(self, R, I):
        # Concatenate reflectance and illumination
        x = torch.cat((R, I), dim=1)

        # Initial convolution
        c1 = self.relu(self.conv1(x)) # Shape: 96x96

        # Downsampling
        down1 = self.relu(self.down_conv1(c1)) # Shape: 48x48
        down2 = self.relu(self.down_conv2(down1)) # Shape: 24x24
        down3 = self.relu(self.down_conv3(down2)) # Shape: 12x12

        # Upsampling and concatenation
        up1 = F.interpolate(down3, size=down2.shape[2:]) # Upsample to 24x24
        up1_conv = self.relu(self.up_conv1(torch.cat((up1, down2), dim=1))) # Concat and apply conv
        
        up2 = F.interpolate(up1_conv, size=down1.shape[2:]) # Upsample to 48x48
        up2_conv = self.relu(self.up_conv2(torch.cat((up2, down1), dim=1)))

        up3 = F.interpolate(up2_conv, size=c1.shape[2:]) # Upsample to 96x96
        up3_conv = self.relu(self.up_conv3(torch.cat((up3, c1), dim=1)))

        # Ensure consistent spatial dimension before concatenation
...
class RetinexNet(nn.Module):
    ...
    def forward(self, x):
        ...
        # Flag for only training Enhance-Net
        ...
        # Flag for only training Decom-Net
