...
def calculate_PSNR(enhanced_img, ground_truth):
    ...
    max_pixel_value = 1.0 # Remember normalized [0;1]
    psnr = 20 * np.log10(max_pixel_value / np.sqrt(mse))
    return psnr

def calculate_SSIM(enhanced_img, ground_truth):
    ssim_metric = SSIM(data_range=1.0) # Remember normalized [0;1]
...
def main():
    ...
    with open('Results/Quantitative results/results.txt_80', 'w') as f:
