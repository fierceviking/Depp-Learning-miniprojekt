...
def main():
    ...
    train_transform = transforms.Compose([
        ...
        transforms.Normalize((0, 0, 0), (1, 1, 1)) # Normalize with min/max
    ...
    if stage == 'decom':
        ...
        model.decom_net.load_state_dict(torch.load(f'models/DecomNet/Job_{num_test}/DecomNet_trained_{num_test}_{num_epochs}.pt'))  #Change back to num_epochs
        
    elif stage == 'finetune':
        model = RetinexNet(train_decom_only=False, train_enhance_only=False).to(device)
        model.decom_net.load_state_dict(torch.load(f'models/DecomNet/Job_{num_test}/DecomNet_trained_{num_test}_100.pt')) #Change back to num_epochs. Be aware of num_test-1 
        model.enhance_net.load_state_dict(torch.load(f'models/EnhanceNet/Job_{num_test}/EnhanceNet_trained_{num_test}_100.pt')) #Change back to num_epochs
    ...
    for epoch in range(num_epochs+1):
        ...
        if epoch % 10 == 0: # Changed to 10 instead of 20 since finetuning only gets 50 epochs
            if stage == 'decom':
